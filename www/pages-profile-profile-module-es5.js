...
var ProfilePage = /** @class */ (function () {
    ...
    ProfilePage.prototype.capturePostImage = function (sourceType) {
        ...
        this.camera.getPicture(options).then(function (imagePath) {
            storageRef = _this.storage.ref(id);
            // Convert url to safe url and set the state of new profileImage
            _this.postImage = _this.webview.convertFileSrc(imagePath);
            console.log('capturePostImage');
            console.log(_this.postImage);
            _this.postImageReady = true;
            // Upload image
            _this.imageID = _this.makeid(60);
            // console.log(this.imageID);
            _this.uploadPostImage(_this.postImage, _this.imageID);
        });
    };
        ...
        return new Promise(function (resolve, reject) {
            ...
            _this.encodeImageUri(imageURI, function (image64) {
                ...
                imageRef.putString(image64, 'data_url')
                    .then(function (snapshot) {
                    resolve(snapshot.downloadURL);
                    _this.postImage = snapshot.downloadURL;
